...
ssh "$PI" 'export PATH=$HOME/.local/bin:$PATH; P=$(ls /dev/serial/by-id/*CP210* | head -1); echo "flashing $P"; sudo systemctl stop companion-hunt; uvx --from esptool esptool.py --chip esp32 --port "$P" --baud 460800 write_flash -z 0x10000 ~/body-fw/firmware.bin 2>&1 | grep -E "Hash of data verified|Hard resetting|error|Failed|fatal" | tail -3; sudo systemctl start companion-hunt; echo "fly restarted (~2 min to load)"'
