...
def cmd_hunt_brain(args, cfg):
    ...
    if args.real:
        ...
        hunter = None if args.scripted else SpikingHunter(cfg, hc, args.load)
        pir = None
        if args.pir:
        ...
        serve_real(cfg, hunter, camera=args.camera or str(cfg.senses.camera.url), cam_fov_deg=float(args.cam_fov), body=_s1_body(args, cfg), pir=pir,
                   port=args.port, open_browser=args.open, rover_on=not args.rover_off, scripted=args.scripted)
        return
    if args.watch:
        from .hunt_gpu.viewer import serve
...
def main(argv=None):
    ...
    p.add_argument("--scripted", action="store_true", help="with --real: no neurons; a plain OpenCV chaser steers at the nearest person and drives into them (the reliability baseline)")
