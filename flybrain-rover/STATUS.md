...
- 05:45 Sat: **Wall visual verified in a real browser** (headless Chromium): Ducks's `hunt_gpu.html` served by `scripts/viz_adapter.py` with our replay → his 3-D scene, person model, our 24 retina cells, DNa02/DNa01 bars, lock/tracking HUD, and our 15,000-neuron frontal brain map with live spikes (`logs/viz_adapter_page.jpg`). His push is missing `ui/fly.js`, `ui/vendor/` (three.js) and `ui/models/`; the adapter serves stand-ins (`replay/assets/vendor`, `replay/assets/models`, a stub fly.js that loads the Body Lab rig). Battery on the S1 read 5 % at 05:40 → must be charged in the hub before any gimbal retest.
- 06:15 Sat: Ducks's branch `s1-fly-brain` (17 commits, unrelated history to main) = his full Companion project: FlyWire brain, ESP32-CAM eye, ESP32 body, and **the S1 driven over S-Bus** (`brain/companion_brain/body/s1.py`, `firmware/sbus-bridge`; measured today: 0.85 m/s, 90°/s at full stick, slow preset). Checked out as a worktree at `../hunting-fly-s1`, nothing pushed. Added `SbusRover` to our bridge (`--s1-sbus PORT`, uses his driver, rotation_only off); verified against a fake serial: correct channels (forward 0.5 → 1300, yaw −1 → 352), 70 Hz, failsafe. This route needs no SDK and no gimbal, so the dead camera board no longer blocks driving. Viewer: with his real `ui/` (fly.js, vendor, models) the adapter shows the Body Lab fly rig with our brain, zero script errors (`logs/viz_his_ui_page.jpg`). Track list saved to TRACKS.md.

### [Sat] The wiring-shuffle control, live on the robot (answers "why not just train a neural net")
`scripts/demo.py` key **6** swaps the running brain between the real connectome and a degree-preserving shuffle built at startup in 0.4 s (2,334,959 synapses: every neuron keeps its exact in-degree, out-degree and signed weights; only the targets are permuted). Key **7** removes LC10a a quarter at a time; **3** restores. Measured on `checkpoints/demo_brain.pt` (person box left or right of a 320x240 frame, 45 frames at 30 Hz, CPU event engine):
| condition | turn | forward | DNa02 L / R | LC10a L / R |
|---|---|---|---|---|
| real, person left | -1.00 | 0.81 | 158.4 / 0.0 | 66.6 / 0.0 |
| real, person right | +1.00 | 1.00 | 0.0 / 251.5 | 0.0 / 64.7 |
| shuffled, person left | 0.00 | 0.05 | 0.0 / 0.0 | 69.9 / 0.0 |
| shuffled, person right | 0.00 | 0.20 | 0.0 / 0.0 | 0.0 / 69.5 |
Graded LC10a (real wiring, person left): 0/25/50/75/100% removed gives DNa02_L 158.4 / 137.5 / 83.4 / 40.3 / 0.0 Hz and turn -1.00 / -1.00 / -1.00 / -0.81 / 0.00; restore returns 158.4 and -1.00 exactly. Reading: under the shuffle the eye is untouched (LC10a still ~70 Hz) and the steering neurons receive nothing, so the demo claim is about the map, not the parts. The graded lesion shows population redundancy: half of LC10a still steers.
Camera is now independent of the S1: `--source` takes a webcam index, a stream URL (ESP32-CAM, phone IP-camera app, RTSP) or a video file, and only a file ends the run; wheels come over `--s1-sbus`. So the dead gimbal board no longer blocks anything.

### [Sat] Camera without the S1's camera, and the Raspberry Pi question answered with numbers
**Eye.** `--source` now takes a webcam index, a stream URL, or a video file, and only a file ends the run. For a stream the bridge uses **Ducks's `CameraStream`** from `hunting-fly` (`brain/companion_brain/senses/camera.py`) when his repo is on disk: it resolves `.local` mDNS names that OpenCV cannot, sets the capture buffer to one frame, reads in a background thread that keeps only the newest frame, and reconnects after a stall. His `read()` downsamples for his optic lobe; the 320x240 BGR frame we need is kept on `.preview`. Verified end to end against a local MJPEG server standing in for the ESP32-CAM: frames flowed, detector 8-11 ms on the Mac GPU, brain 4 ms, **15 ms median camera-to-command**. So vision no longer waits on the S1's dead gimbal board: ESP32-CAM, laptop webcam, or a phone IP-camera app all work, and wheels come over S-Bus.
**Pruning.** `scripts/prune_brain.py` keeps the sensory populations, every descending neuron, the dopamine cells, and the *bridge* (neurons that both receive from the eye and reach a DN): **4,104 neurons (27%) and 408,114 synapses (17%)**. Behaviour survives: M2c PASS (centre symmetric, 169.7 Hz lateral), M3 **77%** turn-toward (full brain 75%), M3b 100% advance, closing 1.84 m to 0.68 m. A 2-hop bridge keeps 99% of the circuit and is pointless. Caveat: Kenyon cells drop out (they are silent in this task), so the pruned file is a runtime circuit, not a mushroom-body training circuit, and `demo_brain.pt` will not load into it (different synapse count).
**Speed.** `scripts/bench_brain.py` on this M2 Pro, batch 1, CPU event engine: full brain **0.68 ms** per 1 ms brain step (14 ms per camera frame at 20 substeps), pruned **0.50 ms** (10 ms per frame). Pruning 73% of the neurons buys only 1.4x, because the event engine never touches a silent neuron: cost is spikes x out-degree, not neuron count (spikes per step barely moved, 101 to 96). So the "prune to 3-5k for a big win" assumption is wrong, and no C rewrite is needed.
**Verdict on the Pi.** A Pi 5 core is roughly 3 to 5x slower than this one, so the pruned brain lands at 1.5 to 2.5 ms per step: **30 to 50 ms per frame at 20 substeps, 15 to 25 ms at 10**. The brain fits on a Pi in plain Python. The blocker is the eye: yolo11n is 8-11 ms on this Mac's GPU but 83 ms on its CPU, so a Pi 5 CPU would be 150-250 ms without an AI HAT or a lighter detector. Recommendation: keep the laptop as brain and eye (15 ms, working now); the Pi is only worth it for the QNX track, and only if the QNX Makerspace can flash it and get a detector running for us. Run `python scripts/bench_brain.py --brain data/brain_pruned.npz` on the actual Pi to replace the estimate with a measurement.
