...
class Exploratory:
    ...
    @torch.no_grad()
    def inject(self, I: torch.Tensor, r: dict, heat_LR: torch.Tensor | None = None,
               blind: bool = False) -> torch.Tensor:
        dt, dev = self.dt, self.device
        # `blind` is the lesioned visual pathway, not a darkened room. The gate below asks whether the
        # brain is being told about a person, and with LC10a/LC4/LPLC2 cut it never is, however bright
        # the retina is. Gating on the retina instead was why a lesioned fly stood still: it 'saw'
        # someone, so it never started searching, and it could not act on them either.
