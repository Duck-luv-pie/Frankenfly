...
Everything known, built, measured, blocked and decided, in one file. Written after an overnight session and a morning of autonomous work. The other documents: `RESULTS.md` is judge-facing evidence, `DEMO.md` is the runbook, `HANDOFF.md` is the teammate quickstart, `DEVPOST.md` is the submission story, `DEVPOST_FIELDS.md` is the form filled in, `STATUS.md` is the dated working log.
...
The public repo carries the product only: code, tests, README, RESULTS, DEMO, DEVPOST, HANDOFF, figures, replay viewer. Working notes (this file, STATUS, CONTEXT, TODAY, PLAN, PITCH, TRACKS, DEVPOST_FIELDS, CLAUDE.md, handoff/, docs/) are gitignored there and stay local.
...
5. **The rented A100's address is still in earlier git history** even though the file is gone from the current tree. Access needs your private key so it is not a credential leak, but the clean fixes are to destroy the instance once the last checkpoints are pulled, or scrub the history with `git filter-repo` and force-push while Ducks is awake.
6. Ducks's latest work has not reached GitHub; his branch head is still the S-Bus commit.
