...
class Brain:
    def __init__(self, checkpoint=None, gain=0.05, device=None, engine="auto", k_t=0.02, k_f=0.3,
        ...
        # The two demo states the viewer drives. `blind` is a real lesion of the visual projection
        # neurons; `halted` is the operator stopping the wheels with the brain left running.
        self.blind = False; self.halted = False; self.blind_speed = 0.30
        self._blind_snap = None; self._blind_idx = None
    ...
    BLIND_GROUPS = ("LC10a_L", "LC10a_R", "LC4_L", "LC4_R", "LPLC2_L", "LPLC2_R")

    @torch.no_grad()
    def set_blind(self, on):
        """Sever the visual projection, so nothing about a person reaches the descending cells.

        lesion(mode="both") zeroes a group's synapses, not the group itself: LC10a, LC4 and LPLC2 keep
        spiking at whatever the retina hands them, and nothing downstream hears a word of it. That is the
        silencing a real LC experiment does, and it is the honest description of this state. Do not call
        it "the neurons are gone" in the demo patter.
        What is left is the exploratory state, and the fly crawls around scanning. That behaviour is not
        scripted: the search drive is internal (brain/explore.py, labelled non-sensory) and it switches
        itself on because the brain has stopped being told anyone is there."""
        on = bool(on)
        if on == self.blind:
            return self.blind
        if on:
            idx = [self.groups[k] for k in self.BLIND_GROUPS if k in self.groups]
            if not idx:
                return self.blind
            self._blind_idx = torch.cat(idx)
            self._blind_snap = self.lif.lesion(self._blind_idx, mode="both")
            if self.explorer is None:            # a blind fly with no search drive would just stand there
                self.explorer = Exploratory(self.groups, self.lif.N, 1, device=self.device, dt=1 / 30)
            self.explorer.reset()
        else:
            if self._blind_snap is not None:
                self.lif.restore(self._blind_snap)
            self._blind_snap = self._blind_idx = None
        self.blind = on
        return self.blind
    ...
    def use_shuffled(self, on):
        ...
        self.lif.reset()
        if self.blind and self._blind_idx is not None:
            # edge snapshots belong to the brain they came from, so re-cut rather than carry one across
            self._blind_snap = self.lif.lesion(self._blind_idx, mode="both")
    ...
    def step(self, boxes_px, heat_LR, now):
        ...
        if self.explorer is not None:
            self.explorer.dt = dt
            self.explorer.inject(I, r, heat, blind=self.blind)
        ...
        if self.lobotomy or self.halted:
            return 0.0, 0.0
        f, t = fwd.item(), turn.item()
        if self.blind:
            f *= self.blind_speed        # it still walks, it just has no reason to walk at you
        return f, t
...
class FakeBox:
    ...
    Each walker is independent: its own pace, its own stretch of the frame, its own distance, and a second
    slower wobble on top so two of them never stay in step. An earlier version gave them one cosine with
    phase offsets and they swept the frame together like a chorus line, which read as scripted rather than
    like a room with people in it. Deterministic (seeded), so a demo looks the same twice.
    """
    def __init__(self, W=320, H=240, period=6.0, n=3, seed=7):
        self.W, self.H, self.t0 = W, H, time.time()
        rng = np.random.default_rng(seed)
        self.walkers = []
        for _ in range(max(1, n)):
            pace = period * float(rng.uniform(0.55, 2.1))          # seconds per sweep, unrelated to the others
            lo = float(rng.uniform(0.05, 0.45))                    # the stretch of frame this one paces
            hi = min(0.95, lo + float(rng.uniform(0.25, 0.6)))
            width = float(rng.uniform(24, 62))                     # apparent size: how far away it is drawn
            self.walkers.append({"pace": pace, "lo": lo, "hi": hi, "w": width,
                                 "phase": float(rng.random()),
                                 "wob_pace": pace * float(rng.uniform(2.3, 4.7)),   # a slower second rhythm
                                 "wob": float(rng.uniform(0.02, 0.07))})

    def __call__(self, frame_bgr=None):
        now = time.time() - self.t0
        out = []
        for k in self.walkers:
            ph = ((now / k["pace"]) + k["phase"]) % 1.0
            u = 0.5 - 0.5 * np.cos(2 * np.pi * ph)                 # 0..1, slow at the turns like a real walker
            u += k["wob"] * np.sin(2 * np.pi * now / k["wob_pace"])
            u = min(1.0, max(0.0, u))
            cx = self.W * (k["lo"] + (k["hi"] - k["lo"]) * u)
            w = k["w"]
            top = 20.0 + (62 - w) * 1.1
...
class VizFeed:
    ...
    def frame(self, t, boxes, fwd, turn, r, heat, exploring):
        ...
        msg = dict(t=round(t - self.t0, 3), forward=round(fwd, 3), turn=round(turn, 3),
                   ...
                   blind=bool(getattr(b, "blind", False)), halted=bool(getattr(b, "halted", False)),
