...
2. **Record the 60-second video**, updated to today's demo (the shuffle, the badge, the voice agent, the Tiger chart). Shot list: 0:00-0:10 the brain map with spikes moving, a person walks in, the robot turns and reaches them ("This is the wiring of a real fruit fly's brain, 15,000 neurons, running live. Nobody trained it."); 0:10-0:22 press 6, the robot sits still while the person moves ("Same neurons, same connections. I only randomize which cell connects to which."), press 6 again and tracking resumes; 0:22-0:32 cut to the badge in hand, swat it, the LEDs flash white ("The same connectome's escape circuit, on this badge, at 100 hertz."); 0:32-0:44 talk to the fly, ask "what do you see?" into the mic and let it answer in its own voice from the live brain state; 0:44-0:54 cut to the wall chart, traces climbing ("Every frame of that goes into a live database as it happens."); 0:54-1:00 "It's a fly. It wants to find you. Nobody programmed that," press space, stop. If the robot is not ready, shoot the same script against `replay/viewer.html` instead and say so in the caption; the badge, voice and Tiger beats do not need the robot at all.
